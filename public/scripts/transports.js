...
Transport.prototype.initDiv = function() {
  ...
  buttonAddTravelHebdo.addEventListener("click", () => {
    this.addTravelRow("trajets-hebdos");
  });

  const buttonAddTravelOccasionel = document.getElementById("add-voyage-occas");
  buttonAddTravelOccasionel.addEventListener("click", () => {
    this.addTravelRow("trajets-occasionels");
  });
};
...
Transport.prototype.computeGES = function() {
  return new Promise((resolve, reject) => {
    ...
    if (this.travels.items.length > 0) {
      ...
      for (let i = 0; i < this.travels.items.length; i++) {
        if (
          this.travels.items[i].depart.getPlace() &&
          this.travels.items[i].arrivee.getPlace()
        ) {
          switch (this.travels.items[i].mode.value) {
            case "Avion":
              travels_pr.push(this.gesAvion(this.travels.items[i]));
              break;
            case "Voiture":
              travels_pr.push(this.gesVoiture(this.travels.items[i]));
              break;
            case "TGV":
              travels_pr.push(
                this.gesTransit(this.travels.items[i], ["TRAIN"])
              );
              break;
            case "Métro/Bus":
              travels_pr.push(
                this.gesTransit(this.travels.items[i], ["BUS", "RAIL"])
              );
              break;
            default:
              console.log(
      ...
      Promise.all(travels_pr).then(values => {
        values.forEach(val => {
