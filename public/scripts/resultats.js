...
class Resultats {
  ...
  constructor(div_id, transport, logement, alimentation, consommation) {
    ...
    this.chart_total = new Chart(
      document.getElementById("ges-chart-total").getContext("2d"),
      {
        type: "doughnut",
        options: {
          responsive: true,
    ...
    this.chart_rank = new Chart(
      document.getElementById("ges-chart-rank").getContext("2d"),
      {
        type: "horizontalBar"
      }
    );
    this.chart_zoom = new Chart(
      document.getElementById("ges-chart-zoom").getContext("2d"),
      {
  ...
  initDiv() {
    ...
    parent_div.innerHTML = `<div class="center">
    ...
    buttonComputeGes.addEventListener("click", () => {
      this.getGes();
    });
   ...
   */
  loadSavedGes() {
    return localStorage.getItem("ges")
      ? JSON.parse(localStorage.getItem("ges"))
      : [];
  }
  ...
  drawGes() {
    ...
    this.chart_total.options.tooltips.callbacks.label = function(
      tooltipItem,
      data
    ) {
      ...
      let tot = data.datasets[tooltipItem.datasetIndex].data.reduce(
        (a, b) => a + b
      );
      label += val;
      label += " kg eq.CO2 (";
    ...
    this.chart_total.options.onClick = e => {
      this.drawGesZoom(e);
    };
    ...
    this.chart_rank.options.scales.xAxes[0] = {
      ticks: { min: 0 },
      scaleLabel: { display: true, labelString: "kg eq.CO2" }
    };
    this.chart_rank.options.scales.yAxes[0] = {
    ...
    this.chart_rank.options.tooltips.callbacks.label = function(
      tooltipItem,
      data
    ) {
      return (
  ...
  drawGesZoom(e) {
    ...
    if (activePoints.length > 0) {
      ...
      let secondColors = this.getSecondaryColors(
        selectedColor,
        Object.keys(secondDataset).length
      );
      ...
      this.chart_zoom.options.tooltips.callbacks.label = function(
        tooltipItem,
        data
      ) {
        return (
